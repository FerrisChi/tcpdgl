...
class DeepwalkDataset:
    def __init__(
        ...
        print(f'Graph has {self.G.num_nodes()} nodes, {self.G.ccg.num_edges() if load_ccg else self.G.num_edges()} edges.')
        ...
        seeds = torch.cat([self.valid_seeds.type(torch.LongTensor)] * num_walks)
