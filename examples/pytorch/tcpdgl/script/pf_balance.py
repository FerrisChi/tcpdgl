    ...
    for prog in PROGRAM:
        for d in DATASET:
            for bal in LOAD_BALANCE:
                out_dir_path = os.path.join(RESULT_PATH, prog.split('.')[0], d)
                out_path = os.path.join(out_dir_path, f"{'pin' if pin else 'gpu'}{'_bal' if bal else ''}")
    ...
    for pin in PIN:
        for prog in PROGRAM:
            for d in DATASET:
                for bal in LOAD_BALANCE:
                    name = 
                    out_path = os.path.join(RESULT_PATH, prog.split('.')[0], d, f"{'pin' if pin else 'gpu'}{'_bal' if bal else ''}")
                    log_path = os.path.join(LOG_PATH, f"{prog.split('.')[0]}_{d}_{'pin' if pin else 'gpu'}{'_bal' if bal else ''}_{cnt}")
...
for prog in PROGRAM:
    for dataset in DATASET:
        for pin in PIN:
            for bal in LOAD_BALANCE:
                d = {}
                out_path = os.path.join(RESULT_PATH, prog.split('.')[0], dataset, f"{'pin' if pin else 'gpu'}{'_bal' if bal else ''}")
                f = open(out_path, 'r')
                ...
                f.write(f"###  {dataset} <{'pin' if pin else 'gpu'}> <{'bal' if bal else ''}>  ###\n")
