...
public:
  ...
  float rand_float(const VertexID_t vertex, const int n) const {
  ...
  __device__ __forceinline__
  double rand_double(const VertexID_t vertex, const EdgePos_t n) const {
  ...
  static EdgePos_t rand_int(curandState* state, const EdgePos_t n) {
    ...
    EdgePos_t id = std::min((EdgePos_t)ff, n-1);//(EdgePos_t)round(ff) - 1.0f;
    return id;
  }

  __device__ __forceinline__
  EdgePos_t rand_neighbor(const VertexID_t vertex, const int n, 
  ...
  {
    cudaMalloc(&d_rand, rand_size*sizeof(float));
    cudaMemcpy(d_rand, rand_nums, rand_size*sizeof(float), cudaMemcpyHostToDevice);

    RandNumGen temp = RandNumGen(rand_nums_per_vertex, num_vertices, d_rand);
    cudaMalloc(&d_ptr, sizeof(RandNumGen));
    cudaMemcpy(d_ptr, &temp, sizeof(RandNumGen), cudaMemcpyHostToDevice);
